...
io.on('connection', socket => {
    ...
    socket.on('agent-register', ({ name }) => {
        const agentName = name || `Helpdesk-${agentCounter++}`;
        agents[socket.id] = { name: agentName, busy: false };

        socket.emit('agent-registered', { name: agentName, queueLength: waitingQueue.length });
    });
    ...
    socket.on('agent-reject', ({ supplierId }) => {
        ...
        // ✅ Notify supplier that agent rejected the call
        io.to(supplierId).emit('call-rejected');
